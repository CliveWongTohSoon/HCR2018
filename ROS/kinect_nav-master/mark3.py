...
#finding contours__________________________________________________________________________
    	unknown,contours, hierarchy = cv2.findContours(edges, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	...
	cv2.createTrackbar('spacing', 'Video', 30, 100, nothing)#for approxPolyDP
		...
		for j in range(cols/spac):
			...
			if (dst[spac*i,spac*j]==80):
				...
				imn=cv2.imread("src/kinect_nav-master/Collision Alert.bmp")
				pub.publish("Collision Alert")
				...
				if(f8==0):
					imn=cv2.imread("src/kinect_nav-master/VCP Reverse.bmp")
					pub.publish("Reverse")
					#print "reverse"
					cv2.imshow('Navig',imn)
				...
				if(f8==0 and f10==0):
					imgshow(flag120,120,imn,'Navig',pub)
				...
				if(f8==0 and f10==0 and f12==0):
					imgshow(flag140,140,imn,'Navig',pub)
			#else:
				#pub.publish("Forwardshahahahah")
